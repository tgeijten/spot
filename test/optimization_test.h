...
namespace spot
{
	void min_progress_test()
	{
		for ( int i = 1; i < 100; ++i )
		{
			...
			opt1.get_stop_condition< min_progress_condition >().progress_window_ = 1000;
			opt1.get_stop_condition< min_progress_condition >().min_progress_ = 1e-6;
			auto* sc1 = opt1.run();

			cma_optimizer opt2( obj, 0, i );
			opt2.get_stop_condition< max_steps_condition >().max_steps_ = 50000;
			auto* sc2 = opt2.run();
